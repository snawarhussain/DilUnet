...
class DilUnet(nn.Module, ABC):
    def __init__(self, input_channels, output_channels):
        ...
        self.ConvTrans2 = nn.ConvTranspose2d(256, 128, kernel_size=(3,3), stride=(2,2), padding=(1,1))
        # self.UpDcBlock2 = DecoderDcBlock(384, 192, 128)
        self.UpDcBlock2 = DecoderDcBlock(256, 192, 128, dilation=1)

        self.ConvTrans3 = nn.ConvTranspose2d(128, 64, kernel_size=3, stride=2, padding=1)
        # self.UpDcBlock3 = DecoderDcBlock(192, 96, 64)
        self.UpDcBlock3 = DecoderDcBlock(128, 96, 64, dilation=1)

        self.ConvTrans4 = nn.ConvTranspose2d(64, 32, kernel_size=3, stride=2, padding=1)
    ...
    def forward(self, X):
        ...
        MultiOutput1 = nn.functional.interpolate(self.MultiOutput1(X6Block), 565)
        MultiOutput2 = nn.functional.interpolate(self.MultiOutput2(X7Block), 565)
        MultiOutput3 = nn.functional.interpolate(self.MultiOutput3(X8Block), 565)
        MultiOutput4 =self.sig(self.output(X9Block))
        out_stacked = torch.cat((.2*MultiOutput1, .5*MultiOutput2,
                                      0.5*MultiOutput3, MultiOutput4), dim=1)

        # out_stacked = torch.cat((MultiOutput1, MultiOutput2,
